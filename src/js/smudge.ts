...
export class Smudge {
  ...
  public clear(material = Material2.clearing): void {
    ...
    _.forEach(bufferLayouts, (_bufferLayout, bufferName) => {
      ...
      this.gl.clearColor(colorRGBA[0], colorRGBA[1], colorRGBA[2], colorRGBA[3]);
  ...
  public export(
    layout: IExportLayout | string = exportLayouts.albedo,
    exportName = `custom_${this.name}`
  ) {
  ...
  private drawGeometery(
    ...
    _.forEach(
      bufferLayouts,
      (drawBuffer = (_bufferLayout: IReadonlyBufferLayout, bufferName: string) => {
        // find the materialChannel for the current buffer
        const materialChannel = material[bufferName];
  ...
  private blit(
    ...
    colorMatrix: Float32Array | number[] = mat4.create(),
    targetBuffer: Framebuffer = null
  ) {
