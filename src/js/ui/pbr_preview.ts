...
export class PBRPreview {
  ...
  constructor(_targetID: string) {
    ...
    if (cubeMatrixString) {
      let cubeMatrix = JSON.parse(cubeMatrixString);
      let copy = this.cube.matrix.fromArray(cubeMatrix);
  ...
  public update(
    ...
    albedo: Framebuffer,
    smoothMetallic: Framebuffer,
    height: Framebuffer,
    emission: Framebuffer
  ) {
...
class MouseDragger {
  ...
  constructor(
    ...
    private requireAlt: boolean,
    private cb: (e: { x: number; y: number; dX: number; dY: number }) => void
  ) {
...
function getThreeTextureForBuffer(
  gl: WebGLRenderingContext,
  buffer: Framebuffer
): THREE.DataTexture {
